...
def simulateBusDCChargingStationIncome(numberOfDCCharger, numberOfBuses, chargingSpeed):
    chargerCost = 15000*numberOfDCCharger
    chargerCost = 0

    busChargingStation = DCChargingStations(numberOfDCCharger)
    busChargingStation.chargeSpeed = chargingSpeed
    busFleet = []
...
def iterate(iteration):
    ...
    for j in range(44,88,1):
        thisBusSwap, thisBusSwapper = simulateBusSwapperIncome(1, j)
        if abs(previousBusSwapper-thisBusSwapper)/thisBusSwapper< 0.005:
            break
        else:
    ...
    j_previous = 44
    for i in range(7,120,5):
        speed = i/60
        for j in range(j_previous-5,j_previous+78,1):
            thisBusCharger, thisBusDCCharger = simulateBusDCChargingStationIncome(23, j,speed)
            if abs(previousBusCharger-thisBusCharger)/thisBusCharger<0.005:
                break
            else:
                previousBusCharger = thisBusCharger
        j_previous = j
        busChargerIncome.append([i,j])
        busSwappingIncome.append([i,j_swapper])
        busDCChargerIncome.append([i,j])
        busSwapperIncome.append([i,j_swapper])
